...
extern "C" unsigned char BitScanForward64_dummy(unsigned long* index,
                                                unsigned long long mask)
{
    unsigned long result = 0;
    if (_BitScanForward(&result, static_cast<unsigned int>(mask))) {
        *index = result;
        return 1;
    } else if (_BitScanForward(&result, static_cast<unsigned int>(mask >> 32))) {
        *index = result + 32;
        return 1;
    } else {
        return 0;
