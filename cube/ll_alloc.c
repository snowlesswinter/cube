...
struct sep
{
	...
	__extension__ union
	{
		...
		{
			unsigned bs_offset; // Offset from the btree which is allocated
                                // directly by system API
                                // (not CONTAINER(btree, data, p)), to the
                                // address indicated by |data| member of btree.
			unsigned size;
		};
		uintptr_t prev;
	};
...
struct btree
{
	...
	__extension__ union
	{
		slist list;   // Used to be linked up in "fast list".
		dlist list2;
		void *data;
...
struct atls
{
	slist fl[NUM_FL];   // Fast-list
	u64b f_mask;        // Fast-list mask.
	...
	dlist qs[NUM_QS]; // Queue for btree nodes that smaller than QS_MAX in size?
...
// Create a separator in a certain point inside the block |block_original|
static inline void set_sep(btree *new_block, int size, btree *block_original)
{
	unsigned offset = block_original->s.bs_offset & ~15;

	/* Store split block offset + size + used indicators */
	new_block->s.bs_offset =
        offset + ((uintptr_t) new_block - (uintptr_t) block_original);
...
static void btree_node_insert(atls *tl, btree *b, int loc, unsigned ssize,
                              btree *new_child)
{
	size_t new_index;
	size_t next_index;
	...
	if (!b_mask(b))
	{
		btree_node_insert_aux(tl, b, loc, ssize, new_child);

		return;
	}

	/* We have space - add it */
	new_index = btree_alloc(b);

	next_index = b_next(b, loc);
	b->bsize[new_index] = ssize + next_index;
	b_prev(b, next_index) = new_index;
	b_next(b, loc) = new_index;
	b_prev(b, new_index) = loc;

	/* Am I last?  Need to update parents */
	if (!next_index) btree_update_psize(b, ssize);

	btree_update_daughter(b, new_child, new_index);
}
...
static void slab_free(atls *tl, void *p)
{
	...
	/* Do I own this? */
	if (unlikely(sb->tail != &tl->tail)) // The slab is from another thread??
                                         // The thread's "tail" has been changed
                                         // after the slab's creation??
	{
		/* Hack wrt mealloc */
...
#else
    // The size increment between slabs are 16. So the index of target slab
    // should be (nsize & ~15) / 16, and since the offset made by shift() is
    // byte-based, we need to specify the size of dlist by:
    // (nsize & ~15) / 16 * 8.
...
// Break the block |b| into two parts. The first part's size is specified by
// |left_node_size|
static always_inline void *split_node(atls *tl, btree *b, size_t total_size,
                                      size_t left_node_size)
{
	size_t right_node_size = total_size - left_node_size;

	btree *sec_block = shift(b, left_node_size);

#ifdef DEBUG_ALLOC_SLOW	
	if (total_size != b->s.size) errx(1, "size missmatch\n");
	if (left_node_size > total_size) errx(1, "too big to fit in split\n");
	check_sep(b);
#endif

	/* Too large to split profitably? */
	if (right_node_size * 4 < left_node_size)
	{
		/* Used this whole */
		return &b->data;
	}
	...
	/* Update local size */
	b->s.size = left_node_size;

	/* Create middle separator */
	set_sep(sec_block, right_node_size, b);

	check_sep(sec_block);

	/* Make sure to try to use remainder */
	fast_free(tl, sec_block, right_node_size);
